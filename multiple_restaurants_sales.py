...
print()
print()
max_rev = dataset.groupby('Manager')['Revenue'].sum().sort_values(ascending=False)
print(max_rev)
...
plt.title('Most Preferred Purchase Type')
plt.show()
...
bar_graph = plt.bar(Most_selling_product.index, Most_selling_product.values, color='tan')
...
# By revenue
plt.figure(figsize = (10,5))
h_bar = plt.barh(By_revenue.index, By_revenue.values, color = 'green')
...
plt.figure(figsize=(10,5))
bar_graph = plt.bar(price.index,price.values, color = 'skyblue')
...
2. Beverages sells the most in quantity due to its low price but generates low revenue. Similarly, Burgers and Fries are mostly favored by the people due to their balanced price as the generate higher reveune and are also one of the most sold products.

3. Chicken Sandwiches are sold less in quantities due to their higher prices. Since quantity and price shows the negative relationship therefore if the restsurant can experiment by lowering the prcies that could increase their selling quantity and generate more revenue.
