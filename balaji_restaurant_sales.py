...
dataset.isnull().sum()
# Null values in transaction type were replaced by Card Payment
...
Time_of_sale_revenue = dataset.groupby('time_of_sale')['transaction_amount'].sum()
print(Time_of_sale_revenue)
# Highest revenue is generated at Afternoon followed by Evening and Midnight
...
# 7) Which gender orders the most?
gender_order = dataset['received_by'].value_counts()
print(gender_order)
# Male order the most when compared to Female

# 8) Which gender order the most food type?
order_per_gender = dataset.groupby('received_by')['item_name'].value_counts()
print(order_per_gender)
# Female and Male mostly order Cold Coffee.
...
2. At afternoon most revenue is generated (Rs 56345) followed by Evening (Rs 52355) and Midnight (Rs 50725).
...
5. Most Preferred transaction is Cash (476 times) followed by Online (417 times) and Card Payment (107 times).

6. Male order the most (512 times) followed by Female(488 times).

7. Male mostly prefer to order Cold Coffee (79 times) followed by Frankie (76 times) and Sugarcane Juice (74 times).

8. Female mostly prefer to order Cold Coffee (82) followed by Sugarcane Juice (79 times) and Panipuri (77 times).
...
2. Male and Female both most prefer Cold Coffee, Sugarcane, and Frankie. Hence, the restaurant should maintain the quality of these prodcuts.

3. Although the price of panipuri is less (20) but it is one of the most sold items i terms of quantites but generates low revenue due to its low price. Therefore, the restaurant can experiment by slightly increasing the price of panipuri and review the performance in terms of quantities sold and revenue generated.
